...
-- Created for dashboard KPI queries and alert/recommendation filtering
-- =============================================================================

-- Scores table — fastest latest scores per product
CREATE INDEX IF NOT EXISTS idx_scores_product_date_desc 
  ON scores (product_id, period_date DESC);

CREATE INDEX IF NOT EXISTS idx_scores_product_period_score
  ON scores (product_id, period_date DESC, performance_score);

-- Alerts table — filter by resolution status and severity
CREATE INDEX IF NOT EXISTS idx_alerts_resolved_severity
  ON alerts (is_resolved, severity);

CREATE INDEX IF NOT EXISTS idx_alerts_product_resolved
  ON alerts (product_id, is_resolved);

-- Recommendations table — filter by acknowledgment and priority
CREATE INDEX IF NOT EXISTS idx_recommendations_ack_priority
  ON recommendations (is_acknowledged, priority);

CREATE INDEX IF NOT EXISTS idx_recommendations_product_ack
  ON recommendations (product_id, is_acknowledged);

-- Raw data table — queries by product and date range
CREATE INDEX IF NOT EXISTS idx_raw_data_product_date_range
  ON raw_data (product_id, period_date DESC, is_validated);

-- Model registry — find active models by type
CREATE INDEX IF NOT EXISTS idx_model_registry_type_active
  ON model_registry (model_type, is_active);

-- Processed features — engineered features lookup
CREATE INDEX IF NOT EXISTS idx_processed_features_quality
  ON processed_features (data_quality_flag, product_id);
