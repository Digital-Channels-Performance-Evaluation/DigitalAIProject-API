...
from app.models.ml_models import Score, ModelRegistry
...
def seed_database():
    ...
    try:
        ...
        # Register ML models in the model registry
        ...
        models_to_register = [
            ...
            {
                "model_name": "Gradient Boosting Classifier v1.0",
        ...
        for model_info in models_to_register:
            ...
            existing = db.query(ModelRegistry).filter(
                ModelRegistry.model_type == model_info["model_type"],
                ModelRegistry.version == model_info["version"]
            ).first()
            ...
            if not existing:
                model_reg = ModelRegistry(
                    ...
                    is_active=(model_info["model_type"] == "xgboost"),  # XGBoost is the default
                    training_date=datetime.now(),
                )
                db.add(model_reg)
        
        db.commit()
        logger.info("Registered ML models in model registry")
