...
@router.post("/compute/{product_id}")
async def compute_score(
    product_id: int,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    """Trigger scoring pipeline for a single product on demand."""
    ...
    # Run feature engineering first in case features are missing
    try:
        feature_engineering_service.reprocess_all(db, product_id)
    except Exception as e:
        raise HTTPException(status_code=400, detail=f"Feature engineering failed: {e}")

    # Check we now have features
    pf = (
    ...
    if not pf:
        raise HTTPException(
            status_code=400,
            detail="No processed features found. Upload data for this product first."
        )

    today = datetime.date.today()
    score_obj = ml_service.score_product(db, product_id, today)
    ...
    recommendation_service.generate_for_product(db, product_id, today, score_obj, features)
    recommendation_service.generate_alerts(db, product_id, today, score_obj, features)

    return {
        "product_id":        product_id,
        "performance_score": score_obj.performance_score,
        "performance_tier":  score_obj.performance_tier,
        "period_date":       str(score_obj.period_date),
