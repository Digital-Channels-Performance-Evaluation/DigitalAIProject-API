...
class RecommendationService:
    ...
    def generate_alerts(
        ...
        # Email managers about new critical/high alerts
        if saved and product:
            self._notify_managers(db, product.name, saved)

        return saved

    def _notify_managers(self, db, product_name: str, alerts: List[Alert]) -> None:
        """Send email digest to all manager-role users for critical/high alerts."""
        from app.services.email_service import send_alert_emails, MANAGER_ROLES
        from app.models.user import User as UserModel
        managers = (
            db.query(UserModel.email)
            .filter(UserModel.role.in_(list(MANAGER_ROLES)), UserModel.is_active == True)
            .all()
        )
        emails = [r.email for r in managers]
        alert_dicts = [
            {"severity": a.severity, "title": a.title, "message": a.message}
            for a in alerts
        ]
        send_alert_emails(emails, product_name, alert_dicts)
